...
static void susfs_terminate_path(char *pathname)
{
...
int susfs_update_sus_kstat(struct st_susfs_sus_kstat* __user user_info) {
	...
	if (susfs_update_sus_kstat_inode(old_info.target_pathname))
		return 1;
	...
	memcpy(&new_entry->info, &old_info, sizeof(new_entry->info));
...
int susfs_add_open_redirect(struct st_susfs_open_redirect* __user user_info) {
	...
	strscpy(new_entry->target_pathname, info.target_pathname,
		SUSFS_MAX_LEN_PATHNAME);
	strscpy(new_entry->redirected_pathname, info.redirected_pathname,
		SUSFS_MAX_LEN_PATHNAME);
...
/*
 * Compatibility helpers for the 4.19 VFS/proc SUSFS hook set.
 * The KernelSU-Next legacy tree does not provide the older helper
 * symbols expected by the susfs4ksu 4.19 kernel patch, so keep them
 * local to fs/susfs.c.
 */

#ifdef CONFIG_KSU_SUSFS_SUS_PATH
int susfs_sus_path_by_path(const struct path *path, int *error,
			   int syscall_family)
{
	struct inode *inode;
...
int susfs_sus_path_by_filename(struct filename *filename, int *error,
			       int syscall_family)
{
	struct path path;
	int ret;
	...
	ret = susfs_sus_path_by_path(&path, error, syscall_family);
	path_put(&path);
...
void susfs_sus_kstat(unsigned long ino, struct stat *stat)
{
	...
	hash_for_each_possible_rcu(SUS_KSTAT_HLIST, entry, node, ino) {
		...
		stat->st_dev = entry->info.spoofed_dev;
		stat->st_ino = entry->info.spoofed_ino;
		stat->st_nlink = entry->info.spoofed_nlink;
		stat->st_size = entry->info.spoofed_size;
		stat->st_blksize = entry->info.spoofed_blksize;
		stat->st_blocks = entry->info.spoofed_blocks;
		stat->st_atime = entry->info.spoofed_atime_tv_sec;
		stat->st_mtime = entry->info.spoofed_mtime_tv_sec;
		stat->st_ctime = entry->info.spoofed_ctime_tv_sec;
#ifdef STAT_HAVE_NSEC
		stat->st_atime_nsec = entry->info.spoofed_atime_tv_nsec;
		stat->st_mtime_nsec = entry->info.spoofed_mtime_tv_nsec;
		stat->st_ctime_nsec = entry->info.spoofed_ctime_tv_nsec;
#endif
		break;
	}
	rcu_read_unlock();
...
int susfs_sus_mount(struct vfsmount *mnt, struct path *root)
{
	...
	r = real_mount(mnt);
	if (r && r->mnt_id >= DEFAULT_SUS_MNT_ID)
		return 1;

	inode = d_inode(mnt->mnt_root);
	if (inode && unlikely(inode->i_state & INODE_STATE_SUS_MOUNT) &&
	    likely(current->susfs_task_state & TASK_STRUCT_NON_ROOT_USER_APP_PROC))
		return 1;
...
#ifdef CONFIG_KSU_SUSFS_TRY_UMOUNT
__weak void ksu_try_umount(const char *mnt, bool check_mnt, int flags, uid_t uid)
{
}
...
bool uid_matches_proc_need_to_reorder_mnt_id(void)
{
	return likely(current->susfs_task_state & TASK_STRUCT_NON_ROOT_USER_APP_PROC);
}

int susfs_get_fake_mnt_id(int mnt_id, int *out_mnt_id,
			  int *out_parent_mnt_id)
{
	return 1;
}

void susfs_add_mnt_id_recorder(struct mnt_namespace *mnt_ns)
{
}
...
#endif /* CONFIG_KSU_SUSFS_SUS_MOUNT_MNT_ID_REORDER */
